"""
The actual seat-watching work: read active watches, dedupe identical lookups,
check the real API, email matches, track token expiry.

This is called from two places during the transition to a more reliable
scheduler:
  - worker.py (root) -- still run by GitHub Actions for now
  - webapp/app.py's /internal/cron route -- the new path, driven by
    cron-job.org

A Postgres advisory lock makes it safe for both to exist at once: if one is
already mid-run, the other just skips rather than double-processing and
sending duplicate emails. Once the new path is proven reliable, worker.py and
the GitHub Actions workflow get retired and this becomes the only caller.
"""
...
from railway_api import TokenExpired, fetch_trains, find_train

# Arbitrary fixed key identifying "the seat-watching cron job" as a single
# mutually-exclusive unit of work, regardless of which process is asking.
CRON_LOCK_KEY = 727271
...
def run_once():
    """Returns a short human-readable status string. Safe to call from
    anywhere -- manages its own DB connection and its own mutual-exclusion
    lock, independent of any caller's context."""
    conn = psycopg2.connect(DATABASE_URL)
    conn.autocommit = True
    cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)

    try:
        cur.execute("SELECT pg_try_advisory_lock(%s) AS locked", (CRON_LOCK_KEY,))
        ...
        # Group identical lookups: many customers watching the same
        # train+date+class cost exactly one API call, not one per customer.
        groups = defaultdict(list)
        for w in watches:
            key = (w["from_city"], w["to_city"], w["date_of_journey"], w["seat_class_param"])
            groups[key].append(w)

        # Guarantee this row exists rather than assuming schema.sql's seed
        # insert landed -- if it's ever missing, this heals it instead of
        # crashing every run.
        ...
        summary = (
            f"ok: {len(groups)} unique search(es), {len(watches)} watch(es), "
            f"{alerts_sent} alert(s) sent"
        )
        cur.execute(
            "UPDATE worker_state SET last_run_at = now(), last_run_ok = true, "
            "last_run_summary = %s WHERE id = 1",
            (summary,),
        )
        return summary
    finally:
